...
class AccountMove(models.Model):
    ...
    @api.depends('state', 'move_type', 'einv_sa_confirmation_datetime', 'einv_sa_confirmed')
    def _compute_einv_sa_confirmation_datetime(self):
        for move in self:
            move.einv_sa_confirmed = False
            print(move.einv_sa_confirmed)
            print(move.einv_sa_confirmation_datetime)
            print(move.country_code)
            if not move.einv_sa_confirmation_datetime and move.state == 'posted' and move.move_type in ('out_invoice', 'out_refund'):
